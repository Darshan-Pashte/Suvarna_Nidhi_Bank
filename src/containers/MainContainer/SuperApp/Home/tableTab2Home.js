...
const columns = [
  ...
  { field: "transNarrative", headerName: "Particulars", width: 530 },
  ...
  },
  {
    ...
    width: 130,
    renderCell: (params) => (
      <span style={{ color: params.row.transDRCR === "C" ? "green" : "red" }}>
        {params.value}
      </span>
    ),
  },
...
export default function DataTable({
  ...
  useEffect(() => {
    const fetchAccountData = async () => {
      try {
        setIsloading(true);
        const response = await axios.post(apiList.FETCHACC, {
          custNo: user.userId,
  ...
  useEffect(() => {
    ...
    if (selectedAccountNo && accList?.length > 0) {
      setinitial(true);
      if (initial) {
        console.log("Run Selected Account");
        fetchData();
      }
    }
  }, [selectedAccountNo]);
    ...
    // },
    {
      ...
      options: {
        ...
        customBodyRender: (value, tableMeta, updateValue) => {
          ...
          // Return a styled component or element
          return <div style={amountStyles}> ₹ {value}</div>;
        },
      },
    },
  ];
        ...
        <div className={classes.Sbox2}>
          ...
          <div style={{ width: "100%", marginBottom: "10px" }}>
            <MUIDataTable
              ...
              data={miniStatementData ? miniStatementData : []}
              columns={columns}
              options={options}
              sx={{
                '& .MuiTableCell-root': {
