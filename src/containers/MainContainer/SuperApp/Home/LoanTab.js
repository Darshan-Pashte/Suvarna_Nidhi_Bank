...
export default function LoanTab({ accList }) {
  ...
  useEffect(() => {
    const fetchLoanData = async () => {
      try {
        setIsloading(true);
        const response = await postApiData(apiList.FETCHLOANINFO, {
          custNo: user.userId,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("loanresponse", response);
        if (response?.status === true) {
          setApiData(response);
          setAtmMasterList(response?.data?.miniStatement);
        } else {
          setAtmMasterList([]);
          SweetAlertPopup(response?.message, "Error", "error");
        }
          ...
          <div className={classes.selectAccount}>
            ...
            <div className={classes.accountNumberSelect}>
              <FormControl fullWidth>
                ...
                >
                  {loanAccounts &&
                    loanAccounts?.data?.loanDeatils.map((item) => (
