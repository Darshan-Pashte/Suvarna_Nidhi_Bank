...
export default function FixedDepositCashCert() {
  ...
  const newListAccount =
    newAccData &&
    newAccData?.map((item) => ({ code: item.brcode, value: item.accountNo }));
  const {
    control,
    handleSubmit,
    setValue,
  ...
  useEffect(() => {
    if (newListAccount) {
      setValue(
        "accno1", compareTextAndReturnObject(newListAccount, newListAccount[0]?.value)
      );
    }
  }, []);
