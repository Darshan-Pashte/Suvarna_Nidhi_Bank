...
const GenerateQRCode = ({ accList }) => {
    ...
    const fetchQRString = async (data) => {
        try {
            ...
            const response = await postApiData(apiList.QRCODEGWNERATE, payload);
    ...
    const fetchQRStringDynamic = async (data) => {
        try {
            ...
            const response = await postApiData(apiList.QRCODEGWNERATEDYANAMIC, payload);
            console.log("qrstring", response)
            if (response?.status == true) {
                setQrResponse(response?.data?.genrateQr);
