...
const CorporateLoans = () => {
  ...
  const { useFormData, defaultFormData, setLoanAccountsCorporate, loanAccountsCorporate, setCustNoContext, custNoContext, } = useContext(ConnContext);
  const {
    control,
    handleSubmit,
    setValue,
  ...
  const loanAccList = loanAccountsCorporate && loanAccountsCorporate?.map(item => ({ "code": item.loanbrCode, "value": item.loanAccNo }));
  ...
  // useEffect(() => {
  //   if (accountList) {
  //     setValue("accountNumber", accountList ? compareIdAndReturnObject(accountList, accountList[0]?.value) : '')
  //   }
  // }, [accountList]);
  ...
  const handleRepayment = async (rowData) => {
    ...
    try {
      ...
      setIsloading(true);
      const response = await postApiData(apiList.CORPORATE_FETCH__REPAYMENT, payload);
      console.log('response', response)
      if (response?.status == true) {
        setLoanDisb(response?.data?.repaymentSchedule);
        navigate("/loans/repaymentdetails", { state: response?.data?.repaymentSchedule })
  ...
  //   if (watch("custNo") != custNoContext) {
  //     handleLoanAccountFetch();
  //     setCustNoContext(watch("custNo"))
  //   }
  // }, [watch("custNo")]);
  ...
  useEffect(() => {
    const custNo = watch("custNo");
    if (custNo && custNo !== custNoContext) {
      handleLoanAccountFetch();
      setCustNoContext(custNo);
    }
  }, [watch("custNo")]);
  ...
  const handleLoanAccountFetch = async () => {
    try {
      ...
      if (response?.status === true) {
        setLoanAccountsCorporate(response?.data?.loanDeatils);
        setIsloading(false);
      } else {
        SweetAlertPopup(response?.message, "Error", "error");
  ...
  const handleDisbursement = async (rowData) => {
    ...
    try {
      ...
      setIsloading(true);
      const response = await postApiData(apiList.CORPORATE_FETCH_LOAN_DISBURSEMENT, payload);
      console.log('response', response)
      if (response?.status == true) {
  ...
  //       const response = await postApiData(
  //         apiList.CORPORATE_FETCH_LOAN_INFO +
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const payload = {
        ...
        sessionId: user?.sessionId,
        fromDate: data.fromDate ? convertDate(data.fromDate, 9) : convertDate(new Date(), 9),
      ...
      const response = await postApiData(apiList.CORPORATE_LOAN_ACCOUNT_STATEMENT_DOWNLOAD, payload);
      // const response = await ServerInstance.post(apiList.ACCOUNT_STATEMENT_DOWNLOAD, payload);
      if (response.status) {
        SweetAlertPopup(response.message, "Success", "success")
        // reset()
        ...
        customBodyRender: (value, { rowData, rowIndex }, tableMeta) => {
          ...
          return (
            <img src={encodedSvg}
              alt="Eye Icon"
              onClick={() => {
                navigate("/loans/loandetails", { state: loanAccountsCorporate[rowIndex] })
              }}
...
<rect width="26" height="26" fill="white"/>
          ...
          const disbursementSvg =
            "data:image/svg+xml," + encodeURIComponent(svgContent);
          return (
            <img src={disbursementSvg}
              alt="Disbursement Icon"
              // onClick={() => navigate("/loans/loandetails",{state:loanList[rowIndex]})}
          ...
          <div className={classes.mainpagePayment}>
            <div className={classes.loaninfo}>
              ...
              <div className={classes.cardsBoxss}>
                ...
                >
                  <Grid item xs={12} sm={6} md={6} style={{ paddingBottom: "2vw" }}></Grid>

                  <Grid item xs={12} sm={6} md={6} style={{ marginLeft: "2vw" }}>
                    <div className={classes.frowdataaff}>
                      <div className={classes.frowtextaff}>
                        Customer Name/No.<sup className={classes.required}>*</sup>
                      </div>
                  ...
                  <Grid item xs={12} sm={12} md={12} style={{ padding: "1vw" }}>
                    <div>
                      <MUIDataTable
                        title={"Loan Account Summary"}
                        data={loanAccountsCorporate ? loanAccountsCorporate : []}
                        columns={columns}
                        options={options}
                      />
                    </div>
                          ...
                          <Grid item xs={12} sm={12} md={12}>
                            <div className={classes.widthtfield}>
                              ...
                              <AutocompleteForm
                                ...
                                rules={{
                                  required:
                                    "Account No." + errorMessages.error_autocomplete_message,
                                  // maxLength: {
...
{/* 
              ...
              <div className={classes.cardsBoxss}>
                ...
                <Box
                  ...
                  className={classes.accountsMarginBox}
                >

                  <div className={classes.FinanceImage}><img src={Finance_app} alt="Finance_app" /></div>
                  <div className={classes.payment1mobileprepaidbutton} >
                    <ColorButton3 onClick={() => navigate("/loans/loanspayment",{state:{loanList:loanAccList}})} variant="contained" type="button">
                      Pay Now
                    </ColorButton3>
                  </div>
                </Box>
