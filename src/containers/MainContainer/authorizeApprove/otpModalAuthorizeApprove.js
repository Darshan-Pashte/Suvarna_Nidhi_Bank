...
export default function OtpCorporateModal({ open, handleOpen, handleClose, userId, payloads, custNo, payloadData, fileData, airtelFileUpload, setAirtelFile , status,response }) {
    ...
    const approveDataList = payloadData.map(item => ({
        rrnNo: item.rrnNo,
    ...
    const onSubmits = async (data) => {
        ...
        const response = await postApiData(apiList.CORPORATE_FUND_TRANFER_APPROVE, payload);
        if (response.status == true) {
            handleClose()
            navigate("/authorizeapprove/authorizersuccesfulpayment", {state : response?.message})
