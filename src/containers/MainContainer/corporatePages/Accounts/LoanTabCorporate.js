...
import classes from "./CorporateAccount.module.css";
...
export default function LoanTabCorporate() {
  const {
    control,
    handleSubmit,
    setValue,
  ...
  const [hitforCust, sethitforCust] = useState(false);
  ...
  const CustDetails =
    ...
    loanAccounts.map((loan) => ({
      code: loan.loanAccNo,
      value: loan.loanAccNo,
      data : loan
    }));

    useEffect(() => {
      if(watch("loanAcc")?.code){
        handleMiniStatement();
      }
    }, [watch("loanAcc")]);
    ...
    const handleMiniStatement = async () => {
      try {
        ...
        let payload = {
          username: user?.userId,
          sessionId: user?.sessionId,
          accNo: watch("loanAcc")?.code,
          brCode: watch("loanAcc")?.data.loanbrCode,
          custNo: watch("custNo")?.code,
          acType: "loan",
        };
        const response = await postApiData(
        ...
        if (response?.status === true) {
          setApiData(response?.data?.miniStatement);
          setIsloading(false);
        } else {
          SweetAlertPopup(response?.message, "Error", "error");
          setApiData([]);
    ...
    // useEffect(() => {
    //   setValue("loanAcc",compareIdAndReturnObject(LoanDetails, LoanDetails[0]?.code));
    // }, [LoanDetails]);
