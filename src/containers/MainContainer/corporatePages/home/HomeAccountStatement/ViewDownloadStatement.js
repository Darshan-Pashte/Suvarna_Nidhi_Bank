...
const CorViewDownloadStatement = () => {
  ...
  const getTransactionListView = async (currentPage) => {
    ...
    try {
      ...
      const response = await postApiData(
        apiList.CORPORATE_PDF_REQUEST_VIEW + `?pageNo=${currentPage}&pageSize=${goPageNumber}`,
        payload
      );
  ...
  const handleDownloadPDF = async (data) => {
    try {
      const response = await fetch(`${apiList.CORPORATE_PDF_DOWNLOAD}/${data[6]}/${data[3]}/${user?.sessionId}`);
...
export default CorViewDownloadStatement;
