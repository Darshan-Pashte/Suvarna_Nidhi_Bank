...
export default function OtpModalDeleteBeneficiary({
  open,
  handleOpen,
  handleClose,
  userId,
  ...
  const onSubmits = async (data) => {
    ...
    const payload = {
      ...
      nickname: payloadData[0],
      sessionId: user?.sessionId,
      otp: otp,
    };
    const response = await postApiData(
      apiList.CORPORATE_BENEFICIARYDELETE,
      payload
    );
  ...
  return (
    <div>
      {loadings ? (<Loader loading={true} />) : (<Loader loading={false} />)}
      <Modal
        open={open}
        // onClose={handleClose}
        aria-labelledby="mod    al-modal-title"
