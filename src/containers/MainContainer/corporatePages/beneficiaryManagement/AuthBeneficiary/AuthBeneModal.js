...
export default function AutheBeneModal({
  open,
  handleClose,
  payloadData,
  status,
  ...
  const onSubmits = async (data) => {
    ...
    const payload = {
      ...
      otp: otp,
      status: status,
      nickname: payloadData.toString(),
    };
    const response = await postApiData(
      apiList.CORPORATE_BENEFICIARY_AUTHORIZE,
      payload
    );
    if (response.status == true) {
      handleClose();
      navigate("/beneficiaryauth/authpayeesuccesfull", {
