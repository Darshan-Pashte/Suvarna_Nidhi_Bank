...
import classes from '../../SuperApp/Home/AccountStatementHome.module.css'
...
const StatementsCorporate = ({ accList }) => {
  ...
  useEffect(() => {
    ...
    if (accountNumber) {
      ...
      if (account) {
        ...
        console.log("accdetails", account)
        setValue('accHolderName', account?.custName);
    ...
    if (diffInDays > 31) {
      ...
      try {
        const payload = {
          ...
          accNo: data.accountNumber.value, // Selected account
          ...
          acType: data?.accHolderName,
        };
  
        const response = await postApiData(apiList.CORPORATE_VIEW_STATEMENT, payload);
  ...
  //         // sessionId: user?.sessionId,
  //         username: user.userId,
  //         accNo: data.accountNumber.value, // Here we are setting the selected account
  //         sessionId: user.sessionId,
  //         brCode: user.accountDetails[0].brCode,
...
export default StatementsCorporate
