...
export default function CorporateFixedDepositInterestRate() {
    ...
    const getDepositAccReceipt = async () => {
      try {
        const payload = {
          ...
          depoType: accountData?.code,


        };
        setIsloading(true)
        const response = await postApiData(apiList.CORPORATE_FETCHDEPOSITINTERESTRATE, payload);
