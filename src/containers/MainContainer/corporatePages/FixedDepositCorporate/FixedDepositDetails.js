...
//   );
const CorporateFixedDepositDetails = () => {
  const {
    control,
    handleSubmit,
    setValue,
  ...
  const getDeposit = async () => {
    try {
      ...
      setIsloading(true);
      const response = await postApiData(apiList.CORPORATE_FETCHDEPOSIT, payload);
      console.log("response", response);
      if (response?.status == true) {

        if (response?.data?.depositesDetails?.length > 0) {
          setDepositList(response?.data?.depositesDetails);
          setIsloading(false);
        } else {

          // SweetAlertPopup("No data available", "Error", "error")
          setDepositList([]);
          setIsloading(false);
        }
      } else {
...
export default CorporateFixedDepositDetails;
