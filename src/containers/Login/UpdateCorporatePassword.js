...
export default function UpdateCorporatePassword({
  open,
  handleClose,
  userId,
}) {
 ...
 const [isLoadin, setIsloadin] = React.useState(false);
  ...
  const onSubmits = async (data) => {
    ...
    const payload = {
      ...
      newpassword: data.newpswrd,
    };
    const response = await postApiData(apiList.CORPORATE_UPDATEPASSWORD, payload);
    if (response.status == true) {
      handleClose();
  ...
  const passwordMatchValidation = (value) => {
    const newPassword = getValues("newpswrd");
      ...
      >
        ...
        >
          <div className={classes.otpContent}>
            <div className={classes.otpheading}>Update Password</div> <div className={classes.cancelButton}>
